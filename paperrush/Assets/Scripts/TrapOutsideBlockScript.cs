...
public class TrapOutsideBlockScript : LevelBlock
{

    public float lengthBeetwenWalls = 20;
    ...
    private Vector3 PlaceForNewCrystalBonus()
    {
        ...
        Side bonusSide = (Side)Random.Range(0, 2);
        float xBonusPosition = Random.Range(-widthWall / 2 + distanceFromWall, -widthEnter / 2);
        float zBonusPosition = Random.Range(zCoordinateBeginningOfBlock + sideOfSquare + 5, zCoordinateBeginningOfBlock + sideOfSquare + lengthBeetwenWalls);
        if (bonusSide == Side.Right)
            xBonusPosition = -xBonusPosition;
